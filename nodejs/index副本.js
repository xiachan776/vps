...
const UUID           = process.env.UUID           || '9c7e2b14-63f8-4a51-bd29-0e6c83f7a412'; // UUID，运行哪吒请修改
...
const ARGO_DOMAIN    = process.env.ARGO_DOMAIN    || 'host-ship.xiachan.kdns.fr';         // argo固定隧道域名,留空即使用临时隧 ?
const ARGO_AUTH      = process.env.ARGO_AUTH      || 'eyJhIjoiNWY3YTRhMTcxMmEzODJhM2M0ZTIzNWNlMzZiZmY4NWUiLCJ0IjoiOWMzYTY3NDYtZmM1NC00NzM1LWE2MDYtZTQxZDNiZjgyZGZiIiwicyI6Ik0yWXpPR0l5TmpVdE0yRXhNeTAwTVdNeUxXRmhZemt0TVdGaE16WmhaV0ZtTXpjNSJ9';         // argo固定隧道token或json,留空即使用临时隧 ?
const ARGO_PORT   = Number(process.env.ARGO_PORT) || 8001;       // argo固定隧道端口
const S5_PORT        = process.env.S5_PORT        || '30221';         // socks5端口，留空不启用
const TUIC_PORT      = process.env.TUIC_PORT      || '';         // tuic端口，留空不启用
const HY2_PORT       = process.env.HY2_PORT       || '30221';         // hy2端口，留空不启用
...
// 前三个变量填齐即启用；不填则整段跳过，对原有节点功能零影响。
// 取值来源：面板「服务器」页 → 复制安装命令 → 对应 -id / -secret / -url 等各参数
// 口径：官方 cf-probe 读 /proc/meminfo、statfs("/")、btime，在容器里拿到的是【宿主机】数字。
//   本段是内置的 cgroup 口径采集器：内存/CPU/磁盘/开机时长全部取本容器的限额与用量。
const CFSM_ID            = process.env.CFSM_ID            || '90e17542-04f2-491e-a360-b169a0fbb3f6';   // -id               服务器ID（每台机器不同，必填）
const CFSM_SECRET        = process.env.CFSM_SECRET        || '19941017';   // -secret           上报密钥 = 面板 API_SECRET
const CFSM_URL           = process.env.CFSM_URL           || 'https://cf-server-monitor.xiachan315.workers.dev/update';   // -url              Worker上报地址(以 /update 结尾)
const CFSM_INTERVAL      = process.env.CFSM_INTERVAL      || '60'; // -interval         上报间隔(秒)
const CFSM_COLLECT_INTERVAL = process.env.CFSM_COLLECT_INTERVAL || '0'; // -collect_interval 高频采样间隔(秒)，0=关
const CFSM_RESET_DAY     = process.env.CFSM_RESET_DAY     || '1';  // -reset_day        月流量重置日(1-31，0=不重置)
const CFSM_CT_NODE       = process.env.CFSM_CT_NODE       || '';   // -ct               电信测速节点 host[:port]
const CFSM_CU_NODE       = process.env.CFSM_CU_NODE       || '';   // -cu               联通测速节点
const CFSM_CM_NODE       = process.env.CFSM_CM_NODE       || '';   // -cm               移动测速节点
const CFSM_BD_NODE       = process.env.CFSM_BD_NODE       || '';   // -bd               BGP测速节点
const CFSM_AGENT_VERSION = process.env.CFSM_AGENT_VERSION || 'local-1.0.0'; // 面板上可一眼看出是哪个采集器
const CFSM_MEM_TOTAL_MB  = process.env.CFSM_MEM_TOTAL_MB  || '';   // 可选：强制内存总量(MB)，默认取 cgroup 限额 / SERVER_MEMORY
const CFSM_DISK_TOTAL_MB = process.env.CFSM_DISK_TOTAL_MB || '';   // 可选：强制磁盘总量(MB)，默认取 SERVER_DISK
const CFSM_DISK_PATH     = process.env.CFSM_DISK_PATH     || '';   // 可选：磁盘用量统计目录，默认脚本所在目录
...
const CFSM_STATE_DIR     = process.env.CFSM_STATE_DIR     || '';   // 可选：状态与日志目录，默认 FILE_PATH/cfsm
...
function cfsmIpFlags() {
  const ifs = os.networkInterfaces();
  let v4 = 0, v6 = 0;
  for (const name of Object.keys(ifs)) {
    if (CFSM_IFACE_EXCLUDE.test(name)) continue;
    for (const a of (ifs[name] || [])) {
      if (a.internal) continue;
      if (a.family === 'IPv4' || a.family === 4) v4 = 1;
      if (a.family === 'IPv6' || a.family === 6) v6 = 1;
    }
  }
  return { v4: v4, v6: v6 };
}
...
async function cfsmReport() {
  const metrics = cfsmBuildMetrics();
  const body = {
    id: CFSM_ID,
    secret: CFSM_SECRET,
    time: Date.now(),
